...
# Load your data
data = pd.read_csv('original_data.csv')  # Replace 'your_data.csv' with your actual file name or path
...
rf_f1_score = f1_score(y_test, rf_predictions, average='micro')  # Add average='micro' to handle multiclass classification
...
lr_f1_score = f1_score(y_test, lr_predictions, average='micro')  # Add average='micro' to handle multiclass classification
...
# Assuming you have already trained and defined rf_model and lr_model
# Apply the preprocessing steps
# itching,hkhk,jlkjk
# input from patient 
frontend = "itching,skin_rash,nodal_skin_eruptions,dischromic _patches"
name = frontend.split(',')
...
for i in range(len(name)):
    try:
        if(features.index('Symptom_'+str(i+1)+'_ ' + name[i]) != -1):
            index[i] = features.index('Symptom_'+str(i+1)+'_ ' + name[i])
    except:
        index[i] = features.index('Symptom_'+str(i+1)+'_' + name[i])

for ind in index:
    new_data[features[ind]] = 1
